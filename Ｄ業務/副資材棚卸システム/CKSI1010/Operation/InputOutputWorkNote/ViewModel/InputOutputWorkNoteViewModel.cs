...
namespace CKSI1010.Operation.InputOutputWorkNote.ViewModel
{
    /// <summary>
    /// 資材班作業誌入力画面(液酸出庫)のビューモデル
    /// </summary>
    public class InputOutputWorkNoteViewModel : OperationViewModelBase
    {
        #region フィールド
        ...
        public InputOutputWorkNoteViewModel(IDataService dataService) : base(dataService)
        {
        }
        ...
        internal override async Task InitializeAsync()
        {
            try
            {
                ...
                FileName = KensinExcelOperator.ReadExcelFilePath(KensinDataType.Shukko);
        ...
        internal override async Task<bool> CommitAsync()
        {
            ...
            try
            {
                if (isExcute)
                {
                    ...
                    // 資材作業誌トランから当月の検針データ(出庫/直送)を取得する
                    ...
                    SharedModel.Instance.MeterDataWorkNoteData.AddRange
                        (await DataService.GetMeterDataWorkNotesAsync
                            (SharedViewModel.Instance.OperationYearMonth.YearMonth.ToString(), 
                                SagyosiKbn.Shukko.GetStringValue(), SagyosiKbn.Chokusou.GetStringValue()));
                        ...
                        foreach (var workNoteItem in SharedModel.Instance.MeterDataWorkNoteData)
                        {
                            ...
                            foreach (var list in itemList)
                            {
                                    ...
                                    workNoteItem.GyosyaCD == list.GyosyaCD)
                                {
                                    ...
                                    if (workNoteItem.Kubun == SagyosiKbn.Shukko.GetStringValue())
            ...
            finally
            {
                if (isExcute)
                {
                    ...
                    await InsertTanaorosiLog(logType, SharedModel.Instance.WorkKbn,
                                             LogOperationType.InputOutputWorkNote,
        ...
        public ICommand FileSelect => new RelayCommand(() =>
        {
            try
            {
                ...
                if ((bool)dialog.ShowDialog())
                {
                    ...
                    KensinExcelOperator.WriteExcelFilePath(KensinDataType.Shukko, FileName);
                }
            }
        ...
        public ICommand ExecuteCKSI0010 => new RelayCommand(async () =>
        {
            ...
            try
            {
                ...
                {
                    BusyStatus.IsBusy = true;
                    ExternalProcessAccessor.StartSizaiSagyosiNyuryoku(KensinDataType.Shukko, FileName);
                    isExcute = true;
                }
                else
                {
