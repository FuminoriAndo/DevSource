...
namespace CKSI1010.Operation.PrintFinancingPresetationInventory.ViewModel
{
    /// <summary>
    /// 財務提出用棚卸表印刷画面のビューモデル
    /// </summary>
    public class PrintFinancingPresetationInventoryViewModel : OperationViewModelBase
    {
        #region フィールド
        ...
        public PrintFinancingPresetationInventoryViewModel(IDataService dataService) : base(dataService)
        {
        }
        ...
        internal override async Task<bool> PrintAsync()
        {
            ...
            finally
            {
                ...
                await InsertTanaorosiLog(logType, SharedModel.Instance.WorkKbn,
                                         LogOperationType.PrintFinancingPresetationInventory,
        ...
        {
            // 財務提出用棚卸表印刷データの一覧
            IList<InventoryFinancingPresetationDTO> outputList = new List<InventoryFinancingPresetationDTO>();
            ...
            try
            {
                ...
                // 立会い用棚卸表印刷データの取得
                var witnessInventoryData = await DataService.GetPrintWitnessModelAsync(tanaYm);
                // 立会い用棚卸表印刷表のデータを整形する
                var shapeData = OperationUtility.ShapeWitnessInventoryData(witnessInventoryData.ToList());

                var workList = shapeData.Where(n => n.HinmokuCD == "????");
                // 資材棚卸立会い用ワークの更新
                await DataService.SetPrintWitnessModelAsync(workList.ToList());

                // 財務提出用棚卸表印刷データの取得
                var financingPresetationData = await DataService.GetPrintFinancingPresetationModelAsync();

                foreach (var item in financingPresetationData)
                {
                    var excelItem = new InventoryFinancingPresetationDTO();
                    // ViewModelからModel(Excel出力用)へプロパティの移送
                    await CopyProperties(item, out excelItem);
                    // 財務提出用棚卸表印刷データの一覧に追加する
                ...
                // 財務提出用棚卸表の出力
                await excelAccessor.OutputFinancingPresetationInventoryList(outputList);
            }
